...
class Login
{
    ...
    public function login_submit()
    {
        ...
        if (empty($admin)) {
            ...
            if (empty($admin)) {
                return to_assign(1, 'ユーザーIDまたは電話番号が間違っています');
            }
        }
        $param['pwd'] = set_password($param['password'], $admin['salt']);
        if ($admin['pwd'] !== $param['pwd']) {
            return to_assign(1, 'ユーザーIDまたはパスワードが間違っています');
        }
        if ($admin['status'] != 1) {
            return to_assign(1, 'アカウントロックされています。 システム管理者にお問い合わせください。');
        }
        $data = [
            'is_lock' => 0,
        ...
        return to_assign(0, 'ログイン成功', ['uid' => $admin['id']]);
    }
