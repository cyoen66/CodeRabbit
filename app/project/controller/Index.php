...
{	
        ...
        if (request()->isAjax()) {
            ...
            $list = ProjectList::withoutField('content,md_content')
                ...
                ->each(function ($item, $key) {
                    ...
                    $item->plan_time = date('Y-m-d', $item->start_time) . ' ~ ' . date('Y-m-d', $item->end_time);
    ...
    public function add()
    {
        ...
        if (request()->isPost()) {
			...
			foreach ($flowNameData as $key => $value) {
				...
				$flowDateData[$key] = str_replace(' ', '',$flowDateData[$key]);
				$flowDate = explode('至',$flowDateData[$key]);
    ...
    public function delete()
    {
        if (request()->isDelete()) {
            ...
            return to_assign(1, "申請内容を確認してください");
        }
    }
}
