        ...
        if ($this->controller !== 'login' && $this->controller !== 'captcha') {
            ...
            if (!Session::has($session_admin)) {
                if ($this->request->isAjax()) {
                    return to_assign(404, 'ログインしてください');
                } else {
                ...
                if (($this->module == 'api') || ($this->module == 'message') || ($this->module == 'home' && $this->controller == 'index')) {
                    ...
                    if (!$this->checkAuth()) {
                        if ($this->request->isAjax()) {
                            return to_assign(405, 'アクセス権限がありません。管理者または人事部に連絡してください。');
                        } else {
                            echo '<div style="text-align:center;color:red;margin-top:20%;">アクセス権がありません。管理者または人事部に連絡してください。</div>';
                            exit;
                        }
                    }
                }
